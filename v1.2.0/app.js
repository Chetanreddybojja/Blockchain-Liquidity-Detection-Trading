...
const txNumberForAntiAutomate = process.env.nrOfTransactionsToDo 
...
let antiAutomateMultiTx = false
if(process.env.antiAutomateActive == 'true'){
    antiAutomateMultiTx = true
}
...
if(dxsalePresale == 1){
  ...
  (async () => {
    ...
    console.log(chalk.red(`WARNING: This instant buy mode detects only listings with presale (DxSale).`))
    console.log(chalk.green(`Connected to blockchain... \n`))
    console.log(chalk.red(`Instant Buy started with current settings:`))
    ...
    console.log(chalk.green(`Total WBNB balance is ${chalk.yellow(parseFloat(ethers.utils.formatUnits(balanceWBNB, 18)).toFixed(6))}\n`))
        ...
        if(buyOnly){
          ...
          if(dxsalePresale == 2){
            console.log('Instant Buy token: '+chalk.green('YES')+' // Fees Multiplication X '+chalk.green(multiply))
          }else{
            console.log('Instant Buy token: '+chalk.green('YES')+' // Listing from '+chalk.green('DxSale'))
          }
        }
  
        if(antiAutomateMultiTx){
          console.log(`AntiAutomate active: `+chalk.green('YES'))
          console.log(`Multiple transactions set to: `+chalk.green(txNumberForAntiAutomate))
        }else{
          console.log(`AntiAutomate active: `+chalk.red('NO'))
    ...
    console.log(chalk.red(`Please press CTRL + C to stop the Automate if the settings are incorect! \n`))
      ...
      if(buyOnly){   
        if(antiAutomateMultiTx){
            ...
            for (i = 0; i < (txNumberForAntiAutomate-1); i++) {
              ...
              console.log(chalk.green(`Start buying token...`)+chalk.yellow(txNumberForAntiAutomate))
              const buytx = await pcsRouterV2B.swapExactTokensForTokens(
              amountIn,
              amountOutMin,
              [WBNB, tokenAddress],
    ...
    dxsaleDetect.on('Mint', async (_0x862d5, _0x4bc59b, _0x4335ba) => {
       console.log( chalk.green('Liquidity detected, starting Instant Buy! '));
  ...
  (async () => {
    currentNonce = await getNonce(myAddress)
    let balanceWBNB = await getWBNBTokenBalance(WBNB, myAddress, provider)
    console.log(chalk.red(`WARNING: This instant buy mode detects only Fair Launch.`))
    console.log(chalk.green(`Connected to blockchain... \n`))
    console.log(chalk.green(`Instant buy started with current settings:`))
    console.log(chalk.green('Buy token for '+chalk.yellow(amountIn/1000000000000000000)+' WBNB using '+chalk.yellow(gasLimit)+' Gas and '+chalk.yellow(gasPrice/1000000000)+' Gwei'))
    console.log(chalk.green(`Total WBNB balance is ${chalk.yellow(parseFloat(ethers.utils.formatUnits(balanceWBNB, 18)).toFixed(6))}\n`))
    if(needApproval){
      console.log(`Approve token: `+chalk.green('YES'))
    ...
    if(buyOnly){
      ...
      console.log('Instant Buy token: '+chalk.green('YES')+' // Fees Multiplication X '+chalk.yellow(multiply))
    }
    
    if(antiAutomateMultiTx){
      console.log(`AntiAutomate active: `+chalk.green('YES'))
      console.log(`Multiple transactions set to: `+chalk.yellow(txNumberForAntiAutomate))
    }else{
      console.log(`AntiAutomate active: `+chalk.red('NO'))
        ...
        if(priceProttection){
          ...
          if(validateLiquidity){
           ...
           await buytx.wait() 
            ...
            console.log(chalk.red(`Waiting for new liquidity, please stop the Automate if you think it's a scam ! (CTRL + C)\n`))
